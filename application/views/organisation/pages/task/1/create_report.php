						...
						<div class="col-lg-6">
							...
							<div class="box box-primary ">
								...
								<div class="box-body" >
												...
												<input type="hidden" class="ngo_payment1" name="ngo_payment1"value="<?php if($val->ngo_payment){ echo $val->ngo_payment; }?>">
													...
													<table id="blog_view_table" class=" table table-striped table-bordered table-hover" >
														...
														<?php if($payment_list){
															...
															?>
															<tr>	
																<td  class="text-center" project_donor_id="<?php echo $pay->project_donor_id;?>" main_donor_id="<?php echo $pay->donor_dropdown_id;?>"><?php echo $pay->donor_dropdown;?></td>
																<td  class="text-center"><i class="fa fa-inr" aria-hidden="true"></i><?php echo $pay->cycle_donor_amount;?></td>
															</tr>
														<?php }}?>
														</tbody>
													</table>
									...
									<div class="cycle_table_error error display_none "  >Atleast one Cycle is required.</div>
												...
												<?php }
													if($project_cycle_donor_data and $project_cycle_donor_data>0){
														foreach($project_cycle_donor_data as $row){
																//var_dump($project_cycle_donor_details_data);
																if($row['donor_amount']!=0){
...
$('document').ready(function(){
			...
			if (is_payment){
				if(is_payment == 'yes'){
					...
					if(ngo_payment){}else{
						...
						var j=0;
						var payment_list = [];
						var allocated_cycle_amount_sum=0;
						$(".cycle_table_append 	.main_table_tr").each(function(key ,value) {
							...
							console.log("j");
							console.log(j);
							console.log("valueggggggggggggggggggggggggggggggg");
							var is_cycle_donor_payment= $(this).find("td:eq(3)").text();
							if(is_cycle_donor_payment=='yes'){
								
								$(".donor_detail_data_appen .donor_detail_tr").each(function(key ,value) {
									var  table_main_donor_id= $(this).find("td:eq()").attr('main_donor_id');
									if(table_main_donor_id){
										var  allocated_cycle_amount= $(this).find("td:eq(1)").text();
										allocated_cycle_amount_sum= allocated_cycle_amount_sum+ parseInt(allocated_cycle_amount);
										console.log("allocated_cycle_amount_sum");
										console.log(allocated_cycle_amount_sum);
										payment_list.push({
											table_main_donor_id :table_main_donor_id ,
											allocated_cycle_amount :allocated_cycle_amount ,		
						...
						$('.donor_detail_data tr').each(function(key,val){
							...
							var main_donor_amount= $(this).find("td:eq(0)").attr('main_donor_amount');
							var donor_amount= $(this).find("td:eq(1) input").val();
										
							console.log(main_donor_amount);
								if(donor_amount){
	...
	$("body").on("click", "#create_cycle", function() {
		...
		if(cycle_list.length==0){
			is_error='yes';
			$(".cycle_table_error").removeClass('display_none');
		}else{
			$(".cycle_table_error").addClass('display_none');
		}
