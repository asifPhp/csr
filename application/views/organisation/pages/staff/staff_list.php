...
if($listdata){
	...
	foreach ($listdata as $key=>$value) {
		...
		for ($jj = 0; $jj < $table_body_column_count; $jj++) {
			...
			}else if($table_body_column[$jj] == 'last_login'){
				if($value[$table_body_column[$jj]]){
					$content .= '<td class="text-center">' . date_time_format($value[$table_body_column[$jj]]). '</td>';
				}else{
					$content .= '<td class="text-center"></td>';
		...
		$content .= '<td class="text-center">';
			...
			//var_dump($staff_role);
			if($staff_role=='Owner' || $staff_role=='Lead Admin'  and $parent_id1!=0){
				if($is_permitted['is_other1'] and $value['parent_id']!=0 ){ 
					$content .= ' <a href="#" data-toggle="modal" data-target="#browsePasswordModule" class="edit_module" name="' . $value[$primary_column_name] . '"  ><span class="label label-primary">Edit Password</span></a>';
				}
			}
			if($value_staff_role !='Owner' and $staff_role!='Owner' and $value_staff_role !='Lead Admin' and $staff_role!='Lead Admin'){
		...
		$content .= '<td class="text-center">';	
			...
			if($value['parent_id']!=0 and $value_staff_role != 'Owner' AND $staff_role!='Owner' and $value_staff_role != 'Lead Admin' AND $staff_role!='Lead Admin'){
				if($is_permitted['is_other2'] ){ 
					if($value_staff_role != 'Owner' AND $staff_role!='Owner' and $value_staff_role != 'Lead Admin' AND $staff_role!='Lead Admin'){
						$content .= '<a href="#" class="define_access" data-toggle="modal" data-target="#browseAccessModal" name="' . $value[$primary_column_name] . '" value=""><span class="label label-info">Define Access</span></a> &nbsp;';
...
<div class="content-wrapper animated fadeInRight">
						...
						<div class="ibox-content">
							...
							<div class="table-responsive">
								<table id="blog_view_table" class="<?php if($listdata){ echo $table_type; } ?> table table-striped table-bordered table-hover" >
									<thead>
										<tr>
											<th class="text-center display_none">Sr no</th>
											<?php
	...
	<?php
	if($sql_query1){
		$listdata2 = $this->db->query($sql_query1)->result_array();
	}else{
		$listdata2 = $this->db->get($table_name1)->result_array();
	}
	/*replace or add with query*/
	...
	if($listdata2){
		...
		foreach ($listdata2 as $key=>$value) {
			...
			for ($jj = 0; $jj < $table_body_column_count1; $jj++) {
				$content .= '<td class="text-center">' . $value[$table_body_column1[$jj]]. '</td>';
			}
			$content .= '<td class="text-center">';
				if($role_name!='Owner'){
					if(isset($edit_url1)){ 
						$content .= '<a href="'.$edit_url1.'?id='.$value[$primary_column_name1].'" class="edit_module"  name="' . $value[$primary_column_name1] . '" value=""><span class="label label-success">Edit</span></a>&nbsp;';
					}
					if(isset($is_remove1)){
						$content .= '&nbsp;<a href="javascript:void(0);" class="remove_item" primary_column_name="'.$primary_column_name1.'"  table_name="'.$table_name1.'" name="' . $value[$primary_column_name1] . '"  ><span class="label label-danger">Remove</span></a>';
					}
				}
			$content .= '</td>';
			$content .= '</tr>';
		...
		$content .= '<tr style="color: red;" class="darker-on-hover"><td class="text-center" colspan="'.($table_body_column_count1+2).'">No data Available</td>';
	}
				...
				<div class="box ">
					<div class="ibox float-e-margins">
						<div class="box-header with-border">
							...
							<?php if(isset($add_url1)){ ?>
								<a style="float: right;" href="<?php echo $add_url1; ?>"  class="btn btn-success pull-right">Add</a>
							<?php }?>
						</div>
						<div class="ibox-content">
							<div id="headerMsg"></div>
							<div class="table-responsive">
								<table id="blog_view_table" class="<?php if($listdata2){ echo $table_type; }?> table table-striped table-bordered table-hover" >
									<thead>
										<tr>
											<th class="text-center">S. No.</th>
											<?php
											if($table_header_column1){
												foreach($table_header_column1 as $val){  
													echo '<th class="text-center">'.$val.'</th>';
												}
											}
											?>
...
$('document').ready(function(){
 	...
 	$('#menu_form').validate({
		...
		messages: {        
 		...
 		submitHandler: function (form) {
            ...
            $.post(APP_URL + 'organisation/staff/update_staff_password', {
            	id: id,
                password: password,
            },
			function (response) {
    ...
    $('body').on('click', '.define_access', function () {
	   ...
	   $.post(APP_URL+'organisation/staff/get_emp_accessibility_detail',{
			staff_id : staff_id,
		},function(response){
			$.unblockUI();
			if(response.status==200){
	...
	$('#access_define_form').validate({
		...
		submitHandler: function (form) {
			...
			*/
            ...
            $.post(APP_URL + 'organisation/staff/update_accessibility', {
                staff_id: staff_id,
			...
			function (response) {
			   ...
			   $('#accessDefineHeadMsg').empty();
				if (response.status == 200) {
                    ...
                    $("#accessDefineHeadMsg").fadeTo(1000, 500).slideUp(500, function(){
						$('#accessDefineHeadMsg').empty();
					}); 
                }				
				$.unblockUI();
