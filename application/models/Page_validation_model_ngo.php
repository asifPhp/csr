...
class Page_validation_model_ngo extends CI_Model {
	...
	public function get_user_access_data($data) {
		...
		return $this->db->get('ngo_staff_access')->result_array();
	}
	/*public function get_user_access_data($user_id) {
		
		$this->db->join('ngo_submodule', 'ngo_submodule.submodule_id = ngo_staff_access.submodule_id');
    ...
    public function get_user_access_data_for_menu($staff_id) {
		...
		$module_data = $this->db->get('ngo_module')->result();
		if ($module_data) {
			$data_value = array();
			foreach ($module_data as $row) {
				...
				$db_result = $this->db->get('ngo_staff_access')->result_array();
				if ($db_result) {
					$data_value1 = array();
					
					$this->db->order_by("submodule_id", "DESC");
					$this->db->where('FK_module_id',$row->module_id);
					//$this->db->where('submodule_id',7);
					$db_result1 = $this->db->get('ngo_submodule')->result();
					if ($db_result1) {
    ...
    public function get_module_sub_module_list_data() {
		...
		if ($module_data) {
			...
			foreach ($module_data as $row) {
				...
				$this->db->where('FK_module_id',$row->module_id);
				$db_result1 = $this->db->get('ngo_submodule')->result_array();
				if ($db_result1) {
					$data_value1 = array();
					foreach ($db_result1 as $row1) {
						$data1 = array();
