...
class Page_validation_model_organisation extends CI_Model {
	...
	public function get_user_access_data($data) {
		...
		return $this->db->get('organisation_staff_access')->result_array();
	}
	/*public function get_user_access_data($staff_id) {
		
		$this->db->join('organisation_submodule', 'organisation_submodule.submodule_id = organisation_staff_access.submodule_id');
     ...
     * @since		1.0.0
     */
    public function get_user_access_data_for_menu($staff_id) {
		$menu_data = array();
		$this->db->order_by("position", "ASC");
		$module_data = $this->db->get('organisation_module')->result();
		if ($module_data) {
			$data_value = array();
			foreach ($module_data as $row) {
				...
				$this->db->where('staff_id',$staff_id);
				$db_result = $this->db->get('organisation_staff_access')->result_array();
				if ($db_result) {
					$data_value1 = array();
					
					$this->db->order_by("submodule_id", "ASC");
					$this->db->where('FK_module_id',$row->module_id);
					$db_result1 = $this->db->get('organisation_submodule')->result();
					if ($db_result1) {
						...
						foreach ($db_result1 as $row1) {
							...
							if (array_key_exists($row1->submodule_id, $data1)) {
								$data1[$row1->submodule_id] = array(
									...
									'link' => $row1->link,
									'icon_class' => $row1->icon_class,
									'menu_link_show' => $row1->menu_link_show,
									'list_link' => $row1->list_link,
									'menu_list_link_show' => $row1->menu_list_link_show,
									'is_add' => $is_add,
									'is_list' => $is_list,
								);
    ...
    public function get_module_sub_module_list_data() {
		...
		if ($module_data) {
			...
			foreach ($module_data as $row) {
				...
				$this->db->where('FK_module_id',$row->module_id);
				$db_result1 = $this->db->get('organisation_submodule')->result_array();
				if ($db_result1) {
					$data_value1 = array();
					foreach ($db_result1 as $row1) {
						$data1 = array();
