$('document').ready(function(){
	...
	$('#gallery_form').validate({
		...
		submitHandler: function (form) {
			...
			$.post(APP_URL + 'admin/configure_access/update_public_gallery', {
			...
			function (response) {
				...
				if (response.status == 200) {
					...
					if(gallery_id!=0){
						...
						window.location.href = APP_URL+'admin/configure_access/add_gallery_with_link';
					});
	...
	$('body').on('click', '.remove_gallery', function () {
        ...
        $.post(APP_URL + 'admin/configure_access/remove_public_gallery', {gallery_id: gallery_id}, function (response) {
            $('#err_testimonials_form').empty();
            if (response.status == 200) {
                $("html, body").animate({scrollTop: 0}, "slow");               
                $('#err_testimonials_form').html("<div class='alert alert-success fade in'>\n\
